...
def symmetry_score(gray, x, y, span):
    ...
    已知局限（2026-09-11 实测，勿轻易改动）：
    * patch 取 span×1.6，背景占比 ~60%，**画面明暗梯度会压低分数**——
      3331-01 底部两枚 mark 因此得 0.64/0.55（去可分离背景后 0.80/0.80），
      但去掉背景梯度后老数据集 20→17 张（0115-03/0519-01/1901-01 反而挂），
      因为它同样会压低背景"恰好帮忙"的标记。净效果为负，故保留原实现。
    * mark 距图像边界 < patch 半径时，REFLECT_101 填充会镜像画面、破坏对称；
      3331-01 的 (229,681) 正是这种情况（差 2 行越界）。"""
