...
**当前版本：0.4.1。** 支持 SE2 实心亮十字；不自动识别 InLens 浮雕风格。
...
标准格式：`0303-1-4-01.tif`。

- `0303` 是数字 marker 编号，前两位是 X 编码、后两位是 Y 编码；乘以 100 后，
  marker 中心为 `(300,300) µm`。
- `1-4` 表示**第 1 行、第 4 列**，末尾 `-01` 是 SEM 自动生成的图片序号。行从上往下、列从左往右，均为 1–4。
- marker 周围的 200×200 µm 区域分为 4×4 个 50×50 µm 小格。
- 该例所属小格左下角为 `(350,350)`、右上角为 `(400,400) µm`。
- 末尾 `-01`、`-02` 是 SEM 自动序号，仅用于区分图片；同一格多张图可用 `0303-1-4-01.tif`、`0303-1-4-02.tif`，序号不参与坐标换算。

编号指四个定位十字围成的方格，不是图片边界。图像向右=CAD +X、向上=+Y；
版图 1 个绘图单位代表 1 µm。完整公式见 [区域编号规范](docs/region-naming.md)。

**新格式已经替换旧规范。CAD 入口拒绝旧点号、`(x,y)` 括号坐标和 `r3c4` 格式；不再读取
坐标覆盖 JSON。** 无效文件名在一键流程开始检测该图之前报告并跳过。
...
按 `0303-1-4-01.tif` 格式命名的原图可以直接运行：
...
文件名格式为 `数字marker编号-行-列-序号.tif`，例如 `0101-3-4-01.tif`。
程序将数字 marker 中心 `(100,100) µm` 和行列号换算为小格左下锚点 `(150,50) µm`，
其余三个十字按固定 50 µm 间距得到设计坐标。无需识别图内数字或大十字。

导出要求质量 PASS、四个中心均为边缘复检结果、原图哈希一致、单位为 px，
且最大单点配准残差不超过 0.05 µm。左下锚点严格固定，其余三点拟合比例和旋转。
拟合残差表示配准一致性，不代表实际曝光套刻精度。

`cad_manifest.json` 记录 `region.marker_code`、`region.row`、`region.column`、
大 marker 中心及小格四周范围；`cad_params.csv` 同时列出编号、行列和换算坐标。
导出报告的 schema_version 为 2。
...
- 不再支持点号区域编号（例如 `0303-1.4.tif`）、`sample(3.5,2).tif` 或尚未实施的 `0101_r3c4_01.tif`。
- `--anchor-overrides` 已移除，旧 `cad_anchor_overrides.json` 不再读取。
- `1-4` 中的第一个数始终是行，第二个数始终是列；不会猜测或交换。
