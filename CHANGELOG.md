# Changelog

## 0.5.1

- `semcorr --batch` **默认生成 AutoCAD 贴图包**，不必再写 `--cad`；`--cad` 仍接受。
- 新增 `--no-cad`：只校正不导出贴图包，文件名可不按区域编号。
- **增量重跑**：原图 SHA256 未变且已有 PASS 时跳过校正；`--force` 强制全量重算。失败/需复核仍不借用历史 PASS。
- 批量结束自动写批次质检 `batch_qc.html/csv/json` 与邻格一致性 `neighbor_checks.csv`；`--qc-only` 可对已有 `corrected/` 单独出报告，`--neighbor-tol-um` 默认 0.05 µm。
- 位置参数传入目录时自动视为 `--batch`。
- 可选 `--dxf`：用 ezdxf 把 SEM 图按 10/11/12 向量写入 DXF 模板并另存 **DXF 2018**（默认 `~/PhD/dxf-gds/RAW/Marker.dxf` → `~/PhD/dxf-gds/DXF/<批次名>.dxf`）；`--dxf-template` / `--dxf-outdir` 可覆盖。默认工作流不变。
- **贴图不建独立图层**：LISP 与 `--dxf` 的 IMAGE 一律放在 `0` 层；重复检测改按图像文件名，`cad_params.csv` 的 `layer` 列改为 `image_id`。

## 0.5.0

- 区域命名中间字段改为**两级象限编号**：`数字marker编号-象限-小象限-序号.tif`，例如 `0719-12-03.tif`。
  第一级把 200×200 µm 大区域取 100×100 µm 象限，第二级在该象限内取 50×50 µm 小格；
  两级都自 `+X/+Y` 起逆时针编号 1–4，第二级方向相对该象限自身的中心判断。
  该例锚点为左下 `(700,1950)`、右上 `(750,2000) µm`。
- 16 个组合恰好铺满大区域，与 0.4.1 行-列命名一一对应（对照表见 `docs/region-naming.md`）；
  坐标换算、锚点约定与贴图流程不变。
- CAD 入口拒绝 `0303-1-4-01` 行-列命名，也拒绝无区域字段的 `0719-01.tif`；不猜测、不自动改名。
- `cad_params.csv` 的 `region_row`/`region_column` 改为 `quadrant`/`sub_quadrant`，
  `cad_manifest.json` 的 `region` 同步改字段并记录象限约定，贴图包 schema_version 升至 3（校正报告仍为 2）。
- 更新 README、区域编号规范和贴图包内说明；测试改为象限命名，新增"16 组合无重叠铺满大区域"
  "与旧行-列坐标一一对应""旧命名与缺字段命名一律拒绝"三项回归。

## 0.4.1

- 区域命名改为 SEM 可直接保存的 `数字marker-行-列-序号.tif`，例如 `0537-2-3-05.tif`。
- 末尾数字序号仅用于区分采集图片，不参与坐标换算；不受行列 1–4 范围限制，且允许省略。
- 同一区域的不同序号仍保留为独立图片和 CAD 图层；拒绝旧点号区域编号，更新使用说明和测试。

## 0.4.0

- CAD 定位规范替换为 `数字marker编号-行.列.tif`，例如 `0303-1.4.tif`；从 200×200 µm 大区域内的 4×4 方格换算绝对坐标。
- 行从上往下、列从左往右，间距固定 50 µm；同格多图可追加 `_01` 等序号。
- 明确拒绝旧括号坐标和 r3c4 格式；移除 `--anchor-overrides`，不再读取旧坐标覆盖文件。
- 一键流程在检测前检查命名；CAD JSON/CSV 保留 marker、行列与换算坐标；CAD 报告 schema_version 升至 2。
- 更新 README 和区域编号规范，增加全部 16 格、越界编号、旧格式拒绝及改名关联测试。

## 0.3.1

- 新增 `semcorr --batch DIR --cad` 一键校正并生成贴图包，支持自定义输出目录和 CAD 参数。
- 统一生成 workflow_summary.json，区分可贴图、校正失败、需复核和 CAD 检查未通过。
- 本次失败或需复核的图片禁止复用历史 PASS 报告，避免重跑时误导出旧结果。
- 保留原有仅校正与独立 CAD 导出入口；不操作 AutoCAD 界面。
- 新增 11 项一键工作流测试，总计 91 项；README 更新项目简介、快速开始、坐标约定和输出说明。

## 0.3.0

- CAD 导出改为文件名括号坐标 ×100 µm 锚定左下十字；拒绝无明确坐标的旧编号推断。
- 固定锚点的相似变换，四点逐点误差检查；检查原图哈希、质量状态、中心来源与单位。
- 原图改名可按 SHA256 关联原校正报告；显式 JSON 坐标覆盖支持人工修正。
- 生成含图像副本的 AutoLISP/SCRIPT 贴图包，设置 IMAGE 像素向量消除 DPI 比例歧义，并计入半像素偏移。
- 图像独立图层、重复运行检查、SEMMAPCHECK 变换核查；不自动保存图形。
- 增加 20 项 CAD 回归用例，总计 80 项测试。

## 0.2.1

- 修复带上下白框的 SEM 信息栏漏裁：检查多个暗行段，忽略孤立尾行，连同上白框裁掉；报告补充实际移除行数。
- 十字中心增加四臂边缘中轴交点精化，降低模板尺寸失配、臂长不一致和亮度分布导致的中心偏移。原图定位与校正图复检均接入。
- 报告记录中心精化方法及边缘拟合证据；只有模板定位的复检中心标记为需复核，CLI 批量汇总单独统计。
- 增加带框信息栏、孤立尾行、重复裁切、亚像素中心、旋转与不等长十字、模板尺度失配、缺臂与圆斑拒绝回归测试。

## 历史开发记录（以下 CAD 原型已被 0.3.0 替代）

### AutoCAD 精确贴图参数生成（新增 scripts/make_cad_align.py）

- 套刻工作流用：从校正结果直接生成把校正图精确贴进版图设计坐标系（µm）的
  IMAGEATTACH 三要素（插入点 / 比例 µm每px / 旋转角）与一键 `.scr` 脚本
  （`cad_params.csv`、`attach_<名>.scr`、`attach_all.scr`），替代手动 ALIGN
  时人眼点击 mark 中心（0.1–0.3 µm 误差 → 拟合残差 ~0.01 µm 量级）。
- 锚定规则：文件名前 4 位按编码解码大十字设计坐标（"0115" → x=100, y=1500 µm）；
  画面内 2×2 = 1 大 + 3 小十字、边长 50 µm；大十字角位由 span 判据在原图上
  自动检测（逐张不同，不能共用模板）。
- 方向自动裁决：错误手性（镜像）组合的相似变换残差达间距量级（~50 µm），
  自动排除；剩余 180° 二义性为"1 大 + 3 小"星座固有（绕大十字转 180° 与
  自身重合），mark 几何不含此信息。脚本默认 图像右=+x、图像下=−y，与用户
  手工"从不翻转旋转"的习惯一致；必要时改脚本顶部 `FORCE_ORIENTATION` 翻转。
- 实测 260910 批次 20 张：拟合残差 0.003–0.033 µm，旋转 ≤0.04°，比例逐张
  0.084–0.100 µm/px（每张倍率不同被逐张吸收，正是逐张拟合的意义）。
- 修正 notebook 过期文案：报告摘要仍指向已废除的 `*_corrected_marked.png`，
  改为 `*_corrected.tif` 内嵌红色小十字。

## 0.2.0

### 校正后中心标注（新增）

- 校正图 `<图像名>_corrected.tif` **就地**用红色小十字标出每个 mark 中心
  （只出一张图，打开即可见）。只做定位指示——不写坐标文字、不画圆圈，
  避免遮挡样品细节。校正图因此为 3 通道 RGB，标记为纯红 `#FF0000` 便于滤除。
- 标记尺寸：缺省臂长**固定 3 px、不随图像尺寸缩放**（常数
  `reporting.MARK_ARM_DEFAULT`），即总宽 7 px、由 **13 个红色像素**组成的
  正十字（7 横 + 7 竖 − 中心 1 个重叠）。臂长 N → 外接框 (2N+1)² px、
  红像素 4N+1 个。
- 新增 CLI 参数 `--mark-arm PX`：以**绝对像素**指定臂长，不再随图像缩放。
  `0` = 只画中心单像素（栅格图像的最小标记）、`1` = 3 px 十字、
  `N` = (2N+1) px 十字，单标记红像素 `4N+1` 个。
  另可通过 `correct_image(mark_arm=N)` 走库接口。
- 修复静默夹取：此前 `--mark-arm 0` 会被 `max(1, ...)` 悄悄变成 1，
  现在 0 表示单像素、负值显式报错。
- 线宽改为**恒为 1 px**（常数 `reporting.MARK_LINE_WIDTH`）。此前默认路径
  会随图像尺寸缩放到 2 px，于是同一张图"传了 `--mark-arm` 线宽 1、不传 2"，
  自相矛盾；现已去掉缩放，线宽与是否传 `arm_px` 无关。
- `draw_center_marks()` 增加越界裁剪，臂长超出画面时不再画出图像外。
- 标记画在 `self_check` **之后**——端到端自检必须在干净的校正图上重新检测。
- 曾短暂改为输出独立的 `<图像名>_corrected_marked.tif`（PNG 版曾放在
  `diagnostics/`），因用户打开 `corrected/` 找不到标记，改回单张内嵌方案。
- 新增 `diagnostics/<图像名>_centers_corrected.csv`（`id,x,y,source,unit`）
  与报告字段 `corrected_centers`，给出**校正图坐标系**下的 mark 中心。
  坐标优先取校正图上重新检测的结果（`source=self-check`），未复检到的
  格位回退到理想格位（`source=ideal-fallback`）。
- 报告 `schema_version` 由 1 升到 2（纯新增字段，旧字段未变）。

### 失败归因（改进）

- 标记数不足时不再建议改 `--grid`，改为**归因到 mark 十字是否残缺**：
  列出已接受标记的三项分数、指出缺失格位及其预测位置、逐项打印恢复门槛
  的通过情况（模板相关 / 臂角形状），并按失败的是哪一项给出成因提示
  （缺臂、被碎屑或套刻图形覆盖、刻蚀不完整、剂量不足、与亮图形粘连等）。
- 新增 `diagnostics/<图像名>_detection_failed.png`：绿圈 = 已接受、
  红叉 = 被图像门剔除、橙圈 = 缺失格位的几何预测位置及搜索范围。
- `recover_missing()` 增加可选 `diag` 出参，返回结构化的失败原因
  （`insufficient_points` / `row_split` / `no_hypothesis` / `gate_rejected`）；
  选优仍只看模板分，但三个分数在选优前一次算齐，失败时证据链完整。
  `diag` 为可选参数，旧调用方式不变。
- 新增 `estimate_spacing()`：用标记点两两最小距离估计网格间距，供诊断
  信息做尺度参照。

### 修复

- 中心坐标标签在四角布局下会互相遮挡（`_centers.png` 与新的
  `_corrected_marked.png` 同病）。新增标签避让布局：右→左→下→上依次试放，
  以压住标记/压住已放标签/越出画面的重叠面积为代价选最优。
- 新增输出文件加入 `OUTPUT_SUFFIXES`，批量处理时不会被当成输入图像重复读入。

### 新增：自动裁掉底部参数栏（新）

- SEM 原图（Zeiss GeminiSEM）底部的深色参数栏会造成：Otsu 被"灰噪声 + 纯黑栏"
  强双峰带偏、栏内白字成为模板匹配的干扰候选、校正后自检失去意义。
  现在**读取后自动检测并裁掉**，无需事先手工裁切。
- 新增模块 `src/semcorr/infobar.py`：`detect_info_bar()` / `strip_info_bar()`。
  四条判据同时成立才认定（保守优先）：行内近黑像素占比 > 25%；条带贴底
  （下沿距底部 ≤ 3% 图高）；高度占 1.5%–20%；底色 p60 近乎纯黑 + 上沿陡降 +
  栏内有近白文字。
  第一条刻意用**行内近黑像素占比**而非行中位数/均值 —— 图越窄白字占行宽比例
  越大，中位数会被抬到 43 以上而把连续段打断（实测踩过）。
- 裁下的条带另存为 `diagnostics/<名>_infobar.png`，实验参数不丢。
- 报告新增 `info_bar` 字段；新增 CLI `--keep-info-bar` 与
  `correct_image(keep_info_bar=...)`。
- 实测：13 张未裁剪原图 **13/13 检出**（条带几何完全一致：上沿 y=707、
  56 行 = 7.3%、上沿落差 255）；22 张已裁好的图**零误裁**。
- 回归测试 38 → 48：新增信息栏检出/不误裁（平坦图、中央暗带、无文字黑带、
  偏暗底色、不同画面宽度）与主流程接入（自动裁、与干净原图结果一致、
  `--keep-info-bar` 生效）。

### 检测器（修复）

- **面积门改为尺度自适应**，替换写死的 `MIN_AREA = 300`：
  `面积门 = max(MIN_AREA_FLOOR=30, 最大可信连通域面积 / MIN_AREA_RATIO=12)`。
  依据是本版图 mark 尺寸固定成族（大十字:小十字 面积比约 8–9），故"最大者 ÷
  常数"在任意倍率下都稳定落在小 mark 之下。旧写死阈值在降倍率时会先丢小十字、
  只留大十字，表现成"标记不齐全"，且极易被误判成"十字形状不完整"。
  实测 0115-01 缩到 80% 时旧规则只剩 1 个标记、自适应仍检全 4 个。
- `find_candidates()` / `detect_marks()` 增加可选 `diag` 出参；报告新增
  `detector` 字段（尺度基准 / 实际面积门 / 形态合格数 / 面积丢弃数），
  verbose 模式打印面积门。旧调用方式不变。
- 失败信息新增反向提示：若粗筛有结构因面积不足被丢弃，明确提示先怀疑
  **成像倍率**，而不是十字残缺。
- **零回归验证**：22 张真实图（20 成功 / 2 失败）逐位复核，标记位置偏移
  `0.00e+00`、自检 RMS 变化 `0.00e+00`、通过集完全一致；全批仅新增 1 个
  候选（0117-01），且被下游三道门拦截。

### 其他

- 移除 Windows 启动器 `semcorr.cmd` 与 CI 的 Windows 矩阵（实验室使用
  场景为 macOS + Jupyter；库代码保持跨平台，文件读写仍显式 UTF-8）。
- 回归测试 27 → 48 个：新增 mark 残缺归因、完整标记不误判、校正图内嵌红标与
  校正后 CSV、校正后中心构成正方形、红色小十字尺寸、`--mark-arm` 覆盖与负值
  拒绝、线宽恒为 1 px、面积门随尺度自适应、降倍率仍检全标记、固定阈值对照、
  信息栏检出与各类不误裁情形。

## 0.1.0

- 冻结原始 SE2 单文件程序。
- 建立 `src/` 布局的可安装项目与 `semcorr` 命令。
- 抽取检测、几何、质量检查、warp、I/O 与报告模块。
- 建立不依赖实验数据的合成回归测试。
- 增加 Windows PowerShell/CMD 原生启动器 `semcorr.cmd`。

## 0.1.1

- 项目从 Kitave-Chain 主仓库拆分为独立仓库（保留提交历史）。
- `correct_image()` / `process_single()` 增加 `verbose` 参数（默认 `True`
  保持 CLI 不变）；`verbose=False` 全程静默，供 notebook 与批量调用。
- 诊断图改用无副作用的 `Figure` API，库不再切换调用方的 matplotlib
  全局后端（notebook 内联绘图不再需要 `%matplotlib inline` 补救）。
- 演示图生成器移入 `semcorr.demo.make_demo_image()`，notebook 与测试
  共用同一实现。
- `semcorr_notebook.ipynb` 重写为精简版：环境自检、参数、静默运行、
  结果图、报告摘要、批量处理六个使用节。
- `correct_image()` 移除从未读取的 `diagnostics` 死参数。
- 移除 `legacy/` 冻结目录（旧版单文件程序仅保留在 git 历史中）。
- 增加 MIT LICENSE；CI 矩阵加入 Windows；README 补充 `--design`
  JSON 格式说明；包版本改为从 `semcorr.__version__` 动态读取。
